...
# Vendored from CGCookie/blender-addon-updater @ 83d8873fc76bdbde8dc8b540b2f075598d21d88d
# Local patch: rolling-tag support for tracking a non-semver GitHub release tag
# (e.g. "latest-main") by reading bl_info["version"] from inside the asset zip.
# Patch markers: search for "ROLLING TAG PATCH".
...
class SingletonUpdater:
    ...
    def __init__(self):
        ...
        # ROLLING TAG PATCH: when set, check_for_update queries the GitHub
        # release with this tag name, downloads its zip asset, and reads the
        # version tuple from the bl_info dict literal in the zip's __init__.py.
        self._rolling_tag = None
        self._rolling_tag_version = None
        self.skip_tag = None
    ...
    # ROLLING TAG PATCH
    @property
    def rolling_tag(self):
        return self._rolling_tag

    @rolling_tag.setter
    def rolling_tag(self, value):
        if value is None or value == "":
            self._rolling_tag = None
        else:
            self._rolling_tag = str(value)
    ...
    # ROLLING TAG PATCH ----------------------------------------------------
    def _rolling_tag_release_url(self):
        """Engine-specific URL for fetching a single release by tag name."""
        if self._engine.name != "github":
            return None
        return "{}/repos/{}/{}/releases/tags/{}".format(
            self._engine.api_url, self._user, self._repo, self._rolling_tag)

    def _rolling_tag_pick_asset(self, release_json):
        if not release_json:
            return None
    ...
    def _read_version_from_zip_bytes(self, zip_bytes):
        """Extract bl_info["version"] from the __init__.py in a zip.

        Uses ast to safely evaluate the bl_info dict literal. Does not exec
        the source. Returns a version tuple like (2, 1, 0) or None on failure.
        """
        try:
        ...
        try:
            ...
            for name in zf.namelist():
                # The zip layout is <pkg>/__init__.py - take the first match.
            ...
            if init_name is None:
                return None
            try:
                src = zf.read(init_name).decode("utf-8", errors="replace")
            except KeyError:
                return None
        finally:
            zf.close()
        ...
        for node in tree.body:
            ...
            if "bl_info" not in target_names:
                continue
    ...
    def _check_rolling_tag(self):
        """Resolve the rolling-tag release into (version_tuple, asset_url).

        Returns None on any failure. Sets self._error / self._error_msg.
        """
        url = self._rolling_tag_release_url()
        if url is None:
            self._error = "Rolling-tag unsupported"
            self._error_msg = (
                "rolling_tag is only implemented for the GitHub engine")
            return None
        self.print_verbose(
            "Rolling tag: fetching release metadata from " + url)
        release = self.get_api(url)
        if not release:
            # get_api populated _error if HTTP/JSON failed; otherwise note it.
            if not self._error:
                self._error = "No release for rolling tag"
                self._error_msg = (
                    "Could not find release for tag '{}'".format(
                        self._rolling_tag))
            return None
        asset_url = self._rolling_tag_pick_asset(release)
        if not asset_url:
            self._error = "Rolling-tag release missing zip asset"
            self._error_msg = (
                "Release for tag '{}' has no .zip asset attached".format(
                    self._rolling_tag))
            return None
        self.print_verbose(
            "Rolling tag: downloading asset to read bl_info version")
        zip_bytes = self._download_bytes(asset_url)
        if zip_bytes is None:
            return None
        version = self._read_version_from_zip_bytes(zip_bytes)
        if version is None:
            self._error = "Rolling-tag version unreadable"
            self._error_msg = (
                "Could not parse bl_info['version'] from rolling-tag zip")
            return None
        return version, asset_url

    def _download_bytes(self, url):
        """Like get_raw, but returns raw bytes instead of decoded text.

        Used to download the rolling-tag zip asset for in-memory inspection.
        """
        request = urllib.request.Request(url)
        try:
            context = ssl._create_unverified_context()
        ...
        try:
            ...
            self._error = "HTTP error downloading rolling-tag asset"
            self._error_msg = str(e.code)
            self.print_trace()
            return None
        except urllib.error.URLError as e:
            self._error = "URL error downloading rolling-tag asset"
            self._error_msg = str(e.reason)
            self.print_trace()
            return None
        try:
    ...
    def check_for_update(self, now=False):
        ...
        # ROLLING TAG PATCH: short-circuit the tag-iteration path when a
        # rolling tag is configured. We fetch one release, peek inside its
        # zip to read bl_info["version"], and decide from that.
        if self._rolling_tag:
            result = self._check_rolling_tag()
            ...
            self._rolling_tag_version = new_version
            if new_version > self._current_version:
                self._update_ready = True
                self._update_version = new_version
                self._update_link = asset_url
                self.save_updater_json()
                return (True, new_version, asset_url)
            self._update_ready = False
            self._update_version = new_version
            self._update_link = asset_url
            return (False, new_version, asset_url)
