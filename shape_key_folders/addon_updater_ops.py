...
updater.addon = "shape_key_folders"
...
def register(bl_info):
    ...
    updater.user = "CamsAvis"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "AI-Slop_ShapekeyFolders"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/CamsAvis/AI-Slop_ShapekeyFolders/"

    # ROLLING TAG PATCH: the updater tracks the "latest-main" rolling release
    # (rather than semver tags). The patched _check_rolling_tag fetches that
    # release, downloads its zip asset, reads bl_info["version"] from inside,
    # and uses that as the candidate version. New main pushes publish a fresh
    # zip on this tag - bump bl_info["version"] before merging to release.
    updater.rolling_tag = "latest-main"
    ...
    # Disabled: we use the rolling-tag path (see updater.rolling_tag above)
    # so branch fetches and tag iteration are bypassed entirely.
